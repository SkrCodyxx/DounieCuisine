...
const dietaryConfig: Record<string, { label: string; icon: LucideIcon; color: string }> = {
  vegetarian: { label: "Vegetarien", icon: Leaf, color: "bg-green-100 text-green-800 border-green-300" },
  vegan: { label: "Vegetalien", icon: Leaf, color: "bg-green-100 text-green-900 border-green-400" },
  ...
  spicy: { label: "Epice", icon: Flame, color: "bg-red-100 text-red-800 border-red-300" },
};

const allergenConfig: Record<string, { label: string; icon: LucideIcon }> = {
    ...
    <div className={`flex flex-wrap gap-2 ${className}`}>
      ...
      {allergensArray.length > 0 && (
        <Badge
          ...
          title={`Allergenes: ${allergensArray
            .map((a) => allergenConfig[a.toLowerCase()]?.label ?? a)
            .join(", ")}`}
        >
          {"Contient: "}
          {allergensArray.slice(0, 2).join(", ")}
